...
router.get('/', (req, res) => {
  ...
  const totalPendingPayments = db.prepare("SELECT COALESCE(SUM(amount),0) value FROM payments WHERE status = 'pending'").get().value;
  const summary = {
    totalViews,
    ...
    totalEstimatedEarnings: isCompanyScope ? totalPendingPayments : calculateEarning(totalViews)
  };
  ...
  const topVideos = db.prepare(`SELECT v.id, v.url, v.platform, v.status, COALESCE(vm.views, 0) views, u.display_name AS influencer_name, p.name AS project_name, ${pendingForUser} AS pending_payment FROM videos v LEFT JOIN video_metrics vm ON vm.id = (SELECT id FROM video_metrics WHERE video_id = v.id ORDER BY scraped_at DESC, id DESC LIMIT 1) LEFT JOIN users u ON u.id = v.owner_user_id LEFT JOIN projects p ON p.id = v.project_id ORDER BY views DESC LIMIT 10`).all()
    .map((row) => ({ ...row, estimated_earnings: isCompanyScope ? row.pending_payment : calculateEarning(row.views) }));
  ...
  const influencers = db.prepare(`SELECT COALESCE(u.display_name, u.username) influencer_name, COUNT(DISTINCT v.id) video_count, COALESCE(SUM(vm.views), 0) total_views, ${pendingForUser} AS pending_payment FROM videos v LEFT JOIN users u ON u.id = v.owner_user_id LEFT JOIN video_metrics vm ON vm.id = (SELECT id FROM video_metrics WHERE video_id = v.id ORDER BY scraped_at DESC, id DESC LIMIT 1) GROUP BY v.owner_user_id ORDER BY total_views DESC`).all()
    .map((row) => ({ ...row, estimated_earnings: isCompanyScope ? row.pending_payment : calculateEarning(row.total_views) }));
  const projects = db.prepare(`SELECT p.name project_name, COUNT(DISTINCT v.id) video_count, COALESCE(SUM(vm.views), 0) total_views, (SELECT COALESCE(SUM(amount),0) FROM payments WHERE project_id = p.id AND status = 'pending') AS pending_payment FROM projects p LEFT JOIN videos v ON v.project_id = p.id LEFT JOIN video_metrics vm ON vm.id = (SELECT id FROM video_metrics WHERE video_id = v.id ORDER BY scraped_at DESC, id DESC LIMIT 1) GROUP BY p.id ORDER BY total_views DESC`).all()
    .map((row) => ({ ...row, estimated_earnings: isCompanyScope ? row.pending_payment : calculateEarning(row.total_views) }));
