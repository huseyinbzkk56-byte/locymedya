...
async function refreshStats(link) {
  ...
  db.prepare('UPDATE links SET stats_views = ?, stats_likes = ?, stats_comments = ?, stats_fetched_at = ? WHERE id = ?')
    .run(metrics.views, metrics.likes, metrics.comments, new Date().toISOString(), link.id);
  if (metrics.title) db.prepare('UPDATE links SET preview_title = ? WHERE id = ?').run(metrics.title, link.id);
  return metrics;
}
...
router.get('/', requireRole('admin'), (req, res) => {
  const links = db.prepare('SELECT * FROM links ORDER BY created_at DESC, id DESC').all();
  res.json({ links });
});

router.post('/', requireRole('admin', 'rapmedia'), (req, res) => {
  ...
  const result = db.prepare('INSERT INTO links (platform, url, title) VALUES (?, ?, ?)').run(platform, url, title || null);
  res.status(201).json({ link: db.prepare('SELECT * FROM links WHERE id = ?').get(result.lastInsertRowid) });
});
...
router.put('/:id', requireRole('admin'), (req, res) => {
  ...
  const result = db.prepare('UPDATE links SET platform = ?, url = ?, title = ? WHERE id = ?').run(platform, url, title || null, req.params.id);
  if (!result.changes) return res.status(404).json({ error: 'Link bulunamadı' });
  res.json({ link: db.prepare('SELECT * FROM links WHERE id = ?').get(req.params.id) });
});
...
router.post('/:id/refresh-stats', requireRole('admin'), async (req, res) => {
  ...
  try {
    ...
    res.json({ link: db.prepare('SELECT * FROM links WHERE id = ?').get(link.id) });
  } catch (err) {
...
router.post('/refresh-stats', requireRole('admin'), async (req, res) => {
  const pending = db.prepare('SELECT * FROM links WHERE stats_fetched_at IS NULL AND archived = 0').all().filter((link) => STATS_PLATFORMS.has(link.platform));
  let updated = 0;
  let failed = 0;
  for (const [index, link] of pending.entries()) {
    try {
      ...
      db.prepare('UPDATE links SET stats_fetched_at = ? WHERE id = ?').run(new Date().toISOString(), link.id);
      failed += 1;
    }
...
router.post('/:id/screenshot', requireRole('admin'), screenshotUpload.single('screenshot'), (req, res) => {
  if (!req.file) return res.status(400).json({ error: 'Ekran görüntüsü zorunlu' });
  const link = db.prepare('SELECT id, screenshot_url FROM links WHERE id = ?').get(req.params.id);
  if (!link) {
  ...
  db.prepare('UPDATE links SET screenshot_url = ? WHERE id = ?').run(screenshotUrl, link.id);
  res.json({ link: db.prepare('SELECT * FROM links WHERE id = ?').get(link.id) });
});

router.delete('/:id/screenshot', requireRole('admin'), (req, res) => {
  const link = db.prepare('SELECT id, screenshot_url FROM links WHERE id = ?').get(req.params.id);
  if (!link) return res.status(404).json({ error: 'Link bulunamadı' });
  if (link.screenshot_url?.startsWith('/uploads/link-screenshots/')) {
    fs.rmSync(path.join(screenshotDir, path.basename(link.screenshot_url)), { force: true });
  }
  db.prepare('UPDATE links SET screenshot_url = NULL WHERE id = ?').run(link.id);
  res.json({ link: db.prepare('SELECT * FROM links WHERE id = ?').get(link.id) });
});

router.post('/:id/archive', requireRole('admin'), (req, res) => {
  const result = db.prepare('UPDATE links SET archived = 1 WHERE id = ?').run(req.params.id);
  if (!result.changes) return res.status(404).json({ error: 'Link bulunamadı' });
  res.json({ link: db.prepare('SELECT * FROM links WHERE id = ?').get(req.params.id) });
});

router.post('/:id/unarchive', requireRole('admin'), (req, res) => {
  const result = db.prepare('UPDATE links SET archived = 0 WHERE id = ?').run(req.params.id);
  if (!result.changes) return res.status(404).json({ error: 'Link bulunamadı' });
  res.json({ link: db.prepare('SELECT * FROM links WHERE id = ?').get(req.params.id) });
});
