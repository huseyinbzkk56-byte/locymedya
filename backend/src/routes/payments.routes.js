...
router.get('/', requireRole('admin'), requireFullAdmin, async (req, res) => {
  ...
  res.json({
    ...
    projects: await db.prepare('SELECT id, name FROM projects ORDER BY created_at DESC').all()
  });
});
...
router.post('/', requireRole('admin'), requireFullAdmin, async (req, res) => {
  ...
  const result = await db.prepare('INSERT INTO payments (influencer_id, media_account_id, project_id, amount, paid_at, note, status) VALUES (?, ?, ?, ?, ?, ?, ?)')
    .run(influencerId || null, mediaAccountId || null, projectId, amount, paidAt || new Date().toISOString(), note || null, status);
  res.status(201).json({ payment: await db.prepare('SELECT * FROM payments WHERE id = ?').get(result.lastInsertRowid) });
});
...
router.put('/:id', requireRole('admin'), requireFullAdmin, async (req, res) => {
  ...
  const result = await db.prepare('UPDATE payments SET influencer_id = ?, media_account_id = ?, project_id = ?, amount = ?, paid_at = ?, note = ?, status = ? WHERE id = ?')
  ...
  res.json({ payment: await db.prepare('SELECT * FROM payments WHERE id = ?').get(req.params.id) });
});
