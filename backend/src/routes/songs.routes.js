...
router.get('/', (req, res) => {
  if (req.user.role === 'admin') return res.json({ songs: db.prepare('SELECT s.*, COALESCE(s.artist_name, a.name) artist_name FROM songs s LEFT JOIN artists a ON a.id = s.artist_id ORDER BY s.created_at DESC').all() });
